...
void GRAFICO(char * ficheiro2)
{
    ...
    listGrafica * vetorMedias[2019] = {NULL};   // vetor de listas com cada uma das cidades (dados importantes)
    ...
    //printCidades(vetorLista, anomin);
    ...
    /*for(int aninho = anomin; aninho <= anomax; aninho++)
    {
        listGrafica * auxi = vetorMedias[aninho]->next;
        while(auxi != NULL)
        {
            printf("%d\t%s\t%f\t%f|%d\t%f|%d\n",aninho, auxi->payload->cidade, auxi->payload->media, auxi->payload->latitude.angular, auxi->payload->latitude.direcao, 
                                            auxi->payload->longitude.angular, auxi->payload->longitude.direcao);
            auxi = auxi->next;
        }
    }*/
        ...
        {   
            ...
            while(SDL_PollEvent(&event))
            {
                ...
                else if(event.type == SDL_MOUSEBUTTONDOWN)
                {
                    ...
                    //printf("MOVING MOUSE: %f|%d : %f|%d\n", longitude, dirLongitude, latitude, dirLatitude);
            ...
            while(aux != NULL)
            {
                ...
                filledCircleRGBA(renderer, calcLongitudePixel(aux->payload->longitude.angular, aux->payload->longitude.direcao),
                            calcLatitudePixel(aux->payload->latitude.angular, aux->payload->latitude.direcao), RAIO, aux->payload->media*255/35, 0, 255-(aux->payload->media*255/35));
...
void leituraficheiro(UNICODE * vetorLista[2019], char * ficheiro2, int * _anomin, int * _anomax)
{
    ...
    while(fgets(buffer, MAX_SIZE, fp) != NULL)
    {
        ...
        if (check == 11)
        {
            ...
            //insert_sorted(payload, vetorLista[ano]);
            insert_tail(vetorLista[ano], payload); // inserir na cauda da lista do respetivo ano
            
            /*if(vetorLista[ano]->next == NULL || vetorLista[ano]->next->payload->dt.ano == ano)
            {
                // adicionar um novo nó na tail da lista
                insert_tail(vetorLista[pos], payload);
            }
            else
            {
                //detetou um novo ano e vai incrementar a posição no vetor e inserir na posição a seguir - REALLOC
                pos++;
                *vetorLista = (UNICODE *) realloc(*vetorLista, pos*sizeof(UNICODE));
                insert_tail(vetorLista[pos], payload);
            }*/
        }
    }
    *_anomin = anomin; // retorna para o main os valores de ano maximo e ano minimo
    *_anomax = anomax;
...
void procuraCidade(SDL_Renderer * renderer, TTF_Font * font, listGrafica *lista [2019], float lat, int dirlat, float longi, int dirlong, int ano, int posx, int posy)
{
    ...
    while(aux != NULL)
    {
        ...
        /*if((aux->payload->longitude.angular == longi) && (aux->payload->longitude.direcao == dirlong) 
                && (aux->payload->latitude.angular == lat) && (aux->payload->latitude.direcao == dirlat))*/
...
{
    if(sqrt(pow(centro_x - ponto_x,2) + pow(centro_y - ponto_y, 2)) <= RAIO) return 1;
